...
delta_L_DCF=1e3;                                % Additional DCF length
...
%% Input signal
t0=2e-12;                                       % Pulse width (unit:ps)
A_input1=exp(-t.^2/(2*t0^2));
A_input2=exp(-(t-10*t0).^2/(2*t0^2));
A_input=A_input1+A_input2;
fA_input1=fft(A_input1);
fA_input2=fft(A_input2);
fA_in=fft(A_input1+A_input2);
figure
plot(t/1e-12,abs(A_input),'r');
xlim([-100,100]);ylim([0,1.2]);
xlabel('Time (ps)');ylabel('Amplitude');
title('Input Temporal signal');
%% Propagation through DCF
fA_incident=fA_input1.*exp(1i*beta2_DCF*L_DCF*(2*pi*f).^2/2)+fA_input2.*exp(1i*beta2_DCF*(L_DCF+delta_L_DCF)*(2*pi*f).^2/2);
...
for mm=2:num+1
    ...
    ti=0e-12;                                    % Timing of GRIN time lens (unit:s)
    I_pump=1-(t-ti).^2/(t_pump^2);                                           
    XPM_opertor=1i*2*gama*P_pump*I_pump;                                              
    u1=A_half.*exp(XPM_opertor*dZ).*rectpuls((t-ti)/(2*t_pump));
    % linear propagation dZ/2
...
for nj=1:length(D0);
    ...
    Aout(nj,:)=ifft(fAout);
    Iout(nj,:)=abs(Aout(nj,:)).^2;
end
Ioutmax=max(max(abs(Iout)));
figure
grid off
mesh(t/1e-12,D0*beta2_SMF/1e-24,Iout/Ioutmax); 
view(0,90);
...
figure
plot(t/1e-12,abs(A_input),'b--');
hold on
plot(t/1e-12,abs(A_image),'r-');
xlim([-60,60]);ylim([0,1.2]);
xlabel('Time (ps)');ylabel('Amplitude');
legend('Input pulse','Output image');
